...
/*   Updated: 2019/10/03 21:46:03 by rjeor-mo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_node(t_list *lst)
{
	...
	ft_printf("id:%d\nname:%s\nx:%d y:%d\nis_end_start:%d\n\n\n", n->id, n->name, n->x, n->y, n->end_start);
}
...
int		fill_node(t_table *t, char **split, int id, int end_start)
{
	...
	if (ft_lstnew(new, sizeof(t_node)) == NULL)
		return (-1);
	ft_lstadd(&t->nl, new);
}
...
int		lm_read(t_table *t)
{
	...
	while (ft_gnl(0, &str, 0) == 0)
	{
		...
		if (flag == 1)
		{
			if (lm_read_node(t, split, comm_tmp) == -1)
			{
				split_free(split);
				free(str);
				ft_error_msg();
